...
import { useSearchSkillsLazyQuery } from "../../../controller";
  ...
  return (
    <MenuItem selected={isHighlighted} component="div" key={suggestion.name}>
      <div>
        {suggestion.id === NEW ? (
          <span>
            [+] Add new skill: <strong>{suggestion.name}</strong>
          </span>
        ) : (
          renderParts(parts)
        )}
...
interface Props {
  updateSelectedValues: (suggestion: ChipsOptionType) => void;
}

function SkillChipsInput(props: Props) {
  ...
  const debounceCallback = useCallback(
    customDebounce((value: any) => {
      callSearch({ variables: { skillName: value } });
    }, 400),
    []
  );
  ...
  const onSuggestionSelected = (
    ...
    setState("");
    props.updateSelectedValues(data.suggestion);
  };

  const [callSearch, { loading, data }] = useSearchSkillsLazyQuery();
  useEffect(() => {
    if (loading) {
      setSuggestions([]);
    } else if (!loading && data && data.searchSkills) {
      if (data.searchSkills.length !== 0) {
        setSuggestions(data.searchSkills);
      } else {
...
export default React.memo(SkillChipsInput);
